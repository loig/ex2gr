...
func initDegreeMatr(correction bool, graphCode, questionCode, answerCode int) (e exo, gCode, qCode int) {
	...
	// title setup
	e.titleImage = titleDegreeMatrImage
	...
	matrixSize := matrixCellSize * (numNodes + 1)
	...
	xshift, yshift := degreeMatrImage.Size()
	...
	e.answers.init((windowWidth-15*answerSize)/2, matrixSize+yshift+3*elementSpacing+20+yTitleShift)
	for i := 0; i < 10; i++ {
