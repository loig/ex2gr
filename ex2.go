...
func initEx2() (e exo) {
	...
	e.drawQuestion = func(screen *ebiten.Image) {
		...
		options.GeoM.Translate(float64((windowWidth-xshift)/2), float64(matrixSize+4*elementSpacing+yTitleShift))
		screen.DrawImage(
			ex2Image,
			&options,
		)
	...
	answerSize := 128
	e.answers.init((windowWidth-3*answerSize)/2, matrixSize+yshift+5*elementSpacing+yTitleShift)
